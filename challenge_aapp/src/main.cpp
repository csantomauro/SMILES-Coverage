...
/*
    #pragma omp critical
    std::cerr << "Thread " << thread_id + 1 << " of " << num_threads
     << "Evaluating ngrams with " << ngram_size << " characters" << std::endl;
         */
    ...
    int ratio = num_words / num_threads;
    if (num_words % num_threads != 0) {
      for (std::size_t word_index = thread_id; word_index < num_words;
           word_index = word_index + num_threads) {
        // std::cerr << "Iteration number:" << word_index << " Out of " << num_words << "Thread number: " <<
        // thread_id +1 << std::endl;
...
#pragma omp critical
      ...
      for (std::size_t word_index = 0 + (thread_id * ratio); word_index < (thread_id + 1) * ratio;
           word_index++) {
        // std::cerr << "Iteration number:" << word_index << " Out of " << num_words << "Thread number: " <<
        // thread_id +1 << std::endl;
...
#pragma omp critical
    ...
    // characters
    /* #pragma omp critical
    std::cerr << "Thread " << thread_id + 1 << " of " << num_threads << ": Current dictionary:" << std::endl;
